...
contract('ERC20Token', function(accounts) {
  ...
  it('initializes the contract with the correct values', function() {
    return ERC20Token.deployed().then(function(instance) {
      ...
      assert.equal(name, 'ERC20 Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'ERCT', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'ERC20 Token v1.0', 'has the correct standard');
    });
  })
  ...
  it('allocates the initial supply upon deployment', function() {
    return ERC20Token.deployed().then(function(instance) {
      ...
      assert.equal(managerBalance.toNumber(), 1000000, 'it allocates the initial supply to the manager account');
    });
  });
